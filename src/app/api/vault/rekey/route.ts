...
import { MAX_REKEY_BODY_BYTES, rotateVaultSchema } from "@/server/vault/contracts";
...
import { rotateVaultTransaction } from "@/server/vault/transaction-store";

export async function PUT(request: NextRequest) {
  try {
    const userId = await getAuthenticatedUserId(request.headers);
    if (!userId) return vaultError(401);
    const body = await parseVaultBody(request, rotateVaultSchema, MAX_REKEY_BODY_BYTES);
    if (!body.success) return body.response;

    const pool = createVaultTransactionPool();
    try {
      const result = await rotateVaultTransaction(pool, userId, body.data);
